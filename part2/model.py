...
Original file is located at
    https://colab.research.google.com/drive/1lfxSb6x0ZpjOo1nkZERu8rUCtqTg5II9
"""
...
class GPT(nn.Module):
    ...
    def generate(
        ...
        for _ in range(max_decoding_time_steps):
            ...
            if do_sample:
                # logits shape: (B, vocab_size) after slicing last time step
                # 1) convert to probabilities
                probs = F.softmax(logits, dim=-1)

                # 2) TOP-K filter (keep only k most likely tokens)
                if top_k is not None and top_k > 0:
                    top_k = min(top_k, probs.size(-1))
                    topk_vals, topk_idx = torch.topk(probs, top_k, dim=-1)
                    # threshold: kth biggest prob for each batch row
                    kth_vals = topk_vals[:, -1].unsqueeze(-1)
                    # zero out everything below the threshold
                    probs = torch.where(probs < kth_vals,
                                        torch.zeros_like(probs),
                                        probs)

                # 3) TOP-P (nucleus) filter (keep smallest set with cum prob >= p)
                if top_p is not None and top_p < 1.0:
                    ...
                    # mask tokens once cumulative prob goes over top_p
                    mask_to_remove = cum_probs > top_p
                    # always keep at least the first (most probable) token
                    mask_to_remove[:, 0] = False

                    # zero out masked probs
                    sorted_probs = sorted_probs.masked_fill(mask_to_remove, 0.0)

                    # scatter back into original order
                    probs = torch.zeros_like(probs).scatter_(1, sorted_idx, sorted_probs)

                # 4) renormalise so rows sum to 1
                probs = probs / probs.sum(dim=-1, keepdim=True)

                # 5) sample one token for each batch element
